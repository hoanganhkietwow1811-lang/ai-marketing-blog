...
import { BLOG_POSTS } from '../data/mockData'; // 👇 Import thêm Mock Data để xem bài cũ
...
interface BlogPost {
  ...
  content: {
    intro: string;
    keyPoints: string[];
    sections: { heading: string; body: string }[]; // Cấu trúc UI cần
    takeaways: string;
  };
  author?: { name: string };
}
...
const PostDetail: React.FC = () => {
  ...
  useEffect(() => {
    const fetchPostDetail = async () => {
      ...
      // 👇 LOGIC QUAN TRỌNG: Kiểm tra xem đây là bài Real (DB) hay Mock (Mẫu)
      const isRealPost = id.startsWith('real-');

      if (isRealPost) {
        // --- TRƯỜNG HỢP 1: BÀI VIẾT TỪ DATABASE ---
        try {
          // 1. Cắt bỏ chữ "real-" để lấy ID gốc (VD: "real-15" -> "15")
          const dbId = id.replace('real-', '');
          
          console.log(`📡 Đang lấy bài từ Server với ID gốc: ${dbId}`);
          ...
          if (foundPostDB) {
            // 2. Chuyển đổi dữ liệu phẳng từ DB sang cấu trúc phân tầng của UI
            setPost({
              id: id, // Giữ nguyên ID trên URL
              ...
              readTime: "5 min read",
              author: { name: "Tri Bui" }, // Hoặc lấy từ foundPostDB.authorId
              content: {
                // Intro lấy đoạn đầu
                intro: foundPostDB.content ? foundPostDB.content.substring(0, 200) + "..." : "",
                keyPoints: ["AI Generated Analysis", "Strategic Overview"],
                // 👇 QUAN TRỌNG: Nhét toàn bộ nội dung vào 1 section để hiển thị đầy đủ
                sections: [
                  { 
                    heading: "Full Analysis", 
                    body: foundPostDB.content // Hiển thị toàn bộ nội dung Markdown
                  }
                ],
        ...
        // --- TRƯỜNG HỢP 2: BÀI VIẾT MẪU (MOCK DATA) ---
        console.log("📂 Đang tìm trong Mock Data...");
        const foundMock = BLOG_POSTS.find(p => p.id === id);
        if (foundMock) {
            // Ép kiểu về interface BlogPost của component này (nếu lệch type)
            // Ở đây mình giả sử cấu trúc Mock Data đã khớp
  ...
  if (!post) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center dark:text-white">
        ...
        <p className="mb-4 text-slate-500">Không tìm thấy bài viết có ID: {id}</p>
        ...
        <div className="prose prose-lg dark:prose-invert max-w-none">
            {post.content.sections.map((sec, idx) => (
                <div key={idx} className="mb-10">
                    {/* Chỉ hiện heading nếu không phải là Full Analysis (để đỡ lặp) */}
                    {sec.heading !== "Full Analysis" && <h3>{sec.heading}</h3>}
                    
                    {/* 👇 SỬ DỤNG white-space-pre-wrap ĐỂ GIỮ XUỐNG DÒNG CỦA AI */}
